...
function findFirstSingleChar(str) {
  ...
  const firstSingleChar = Array.from(str).find((char) => charMap.get(char) === 1);
...
function isCreditCardNumber(ccn) {
  ...
  const reversedDigits = Array.from(ccnStr).reverse();
...
function getDigitalRoot(num) {
  ...
  while (currentNum > 9) {
    ...
    currentNum = digits.reduce((sum, digit) => sum + digit, 0);
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  function checkDirectory(directory) {
    if (pathes.every((path) => path.startsWith(`${commonPath + directory}/`))) {
...
function evaluateTicTacToePosition(position) {
  ...
  for (let row = 0; row < 3; row += 1) {
    if (position[row][0] && position[row][0] === position[row][1]
